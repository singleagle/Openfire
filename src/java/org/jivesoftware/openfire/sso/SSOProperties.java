...
public class SSOProperties {
	...
	public SSOProperties() throws IOException {
		props = new Properties();
		
		File file = new File(JiveGlobals.getHomeDirectory() + File.separator + "conf" + File.separator + "sso-service.properties");
		if (!file.exists()) {
			throw new IOException("The file sso-service.properties is missing from Openfire conf folder");
		} else {
			try {
		...
		if (StringUtils.isBlank(props.getProperty(APPLICATION_NAME))
				...
				|| StringUtils.isBlank(props.getProperty(SSO_SERVER_URL))) {
			
			throw new IOException("sso-service.properties is missing required information (app name, app passwd, crowd url)");
		}
	}
	...
	public String getSSOServerUrl() {
		String url = props.getProperty(SSO_SERVER_URL);
		if (!url.endsWith("/")) {
			url += "/";
		}
		return url;
