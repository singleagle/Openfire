 ...
 */
public class VenusVCardProvider extends DefaultVCardProvider {
	private static final Logger LOG = LoggerFactory.getLogger(VenusVCardProvider.class);
	
	private static final String VCARD_TEMPLATE
		= "<vCard xmlns=\"vcard-temp\"><FN>@displayname@</FN><NICKNAME>@nickname@</NICKNAME>"
				+ "<TEL><CELL/><HOME/><NUMBER>@cellphone@</NUMBER></TEL></vCard>";
	...
	public Element loadVCard(String username) {
		...
		try {
			...
			if (vcard == null) {
				VenusUserProvider userProvider = (VenusUserProvider) UserManager.getUserProvider();
				try {
					VenusUser user = userProvider.getVenusUser(username);
					String str = VCARD_TEMPLATE.replace("@displayname@", String.valueOf(user.getUin()))
							.replace("@cellphone@", user.getPhoneNO())
	...
	public Element updateVCard(String username, Element vCard) throws NotFoundException {
		...
		Element phoneNoNode = vCard.element("CELL");
		...
		try {
			...
			displayNameNode.setText(String.valueOf(user.getUin()));
			phoneNoNode.setText(user.getPhoneNO());
		} catch (UserNotFoundException unfe) {
